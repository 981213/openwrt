...
static unsigned long sf21a6826_cmnpll_vco_recalc_rate(struct clk_hw *hw,
...
static struct sf_clk_common cmnpll_vco = SF_CLK_COMMON(
	"cmnpll_vco", clk_pll_parents, &sf21a6826_cmnpll_vco_ops, 0);
...
static long sf21a6826_cmnpll_postdiv_round_rate(struct clk_hw *hw,
						unsigned long rate,
						unsigned long *parent_rate)
{
	unsigned int diva, divb;

	return sf21a6826_dualdiv_round_rate(rate, *parent_rate, 7, &diva,
					    &divb);
}

static int sf21a6826_cmnpll_postdiv_set_rate(struct clk_hw *hw,
					     unsigned long rate,
					     unsigned long parent_rate)
{
	struct sf_clk_common *priv = hw_to_sf_clk_common(hw);
...
sf21a6826_cmnpll_postdiv_recalc_rate(struct clk_hw *hw,
				     unsigned long parent_rate)
{
	struct sf_clk_common *priv = hw_to_sf_clk_common(hw);
	u32 cfg = sf_readl(priv, PLL_CMN_CFG1);
...
static struct sf_clk_common cmnpll_postdiv =
	SF_CLK_COMMON("cmnpll_postdiv", clk_cmnpll_postdiv_parents,
		      &sf21a6826_cmnpll_postdiv_ops, 0);
...
static unsigned long
sf21a6826_ddrpll_postdiv_recalc_rate(struct clk_hw *hw,
				     unsigned long parent_rate)
{
	struct sf_clk_common *priv = hw_to_sf_clk_common(hw);
...
static struct sf_clk_common ddrpll_postdiv = SF_CLK_COMMON(
	"ddrpll_postdiv", clk_pll_parents, &sf21a6826_ddrpll_postdiv_ops, 0);
...
static unsigned long
sf21a6826_pciepll_vco_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct sf_clk_common *priv = hw_to_sf_clk_common(hw);
...
static struct sf_clk_common pciepll_vco =
	SF_CLK_COMMON("pciepll_vco", clk_pll_parents,
		      &sf21a6826_pciepll_vco_ops, CLK_SET_RATE_GATE);
...
static int sf21a6826_pciepll_fout_enable(struct clk_hw *hw)
{
	...
	struct sf21a6826_pciepll_fout *priv =
		container_of(cmn_priv, struct sf21a6826_pciepll_fout, common);
	unsigned long flags;
...
static long sf21a6826_pciepll_fout_round_rate(struct clk_hw *hw,
					      unsigned long rate,
					      unsigned long *parent_rate)
{
	unsigned int diva, divb;

	return sf21a6826_dualdiv_round_rate(rate, *parent_rate, 8, &diva,
					    &divb);
}

static int sf21a6826_pciepll_fout_set_rate(struct clk_hw *hw,
					   unsigned long rate,
					   unsigned long parent_rate)
{
	struct sf_clk_common *cmn_priv = hw_to_sf_clk_common(hw);
...
static unsigned long
sf21a6826_pciepll_fout_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct sf_clk_common *cmn_priv = hw_to_sf_clk_common(hw);
	struct sf21a6826_pciepll_fout *priv =
		container_of(cmn_priv, struct sf21a6826_pciepll_fout, common);
...
static unsigned long sf21a6826_muxdiv_recalc_rate(struct clk_hw *hw,
						  unsigned long parent_rate)
{
	struct sf_clk_common *cmn_priv = hw_to_sf_clk_common(hw);
	struct sf21a6826_clk_muxdiv *priv =
		container_of(cmn_priv, struct sf21a6826_clk_muxdiv, common);
...
static int sf21a6826_muxdiv_determine_rate(struct clk_hw *hw,
...
static int sf21a6826_muxdiv_set_rate(struct clk_hw *hw, unsigned long rate,
				     unsigned long parent_rate)
{
	struct sf_clk_common *cmn_priv = hw_to_sf_clk_common(hw);
	struct sf21a6826_clk_muxdiv *priv =
...
static int sf21a6826_mux_determine_rate(struct clk_hw *hw,
					struct clk_rate_request *req)
{
